...
def calc_parallel_perm_mask(sigma, step, seqlen, select_conditioning):
    ...
    assert seqlen == sigma.shape[-1]
    mod_sigma[sigma >= step] = seqlen # Only condition the new tokens on the ones we've already decoded
    perm_mask = mod_sigma.unsqueeze(2) <= mod_sigma.unsqueeze(1) # Ban attention to later items, as well as self-attention.
    perm_mask[select_conditioning] = 0 # Condition on all the initially visible tokens (still principled, as long as we never evaluate the density of the conditioning)
    return mod_sigma, perm_mask

# Calculates the permutation mask for the GT computation (no parallelism)
def create_gt_perm_mask(sigma, seqlen, select_conditioning):
    perm_mask = sigma.unsqueeze(2) <= sigma.unsqueeze(1) # Ban attention to later items, as well as self-attention. https://github.com/zihangdai/xlnet/blob/bbaa3a6fa0b3a2ee694e8cf66167434f9eca9660/data_utils.py#L626
    perm_mask = perm_mask.to(torch.float)
    assert perm_mask.shape == (1, seqlen, seqlen)
            ...
            with torch.no_grad():
                for idx_s in range(k):
                    ...
                    if n_gram_query == mask_token:
                        ...
                        assert order_to_pos[n+idx_s-1] == curr_idx - 1 # make sure prev token is masked because it's the one we just speculated
...
def update_context_ngram(context_ngram, new_sequence, idx_in_seq, seqlen):
    if idx_in_seq > 0: # add the bigram that ends here
        context_ngram.update(
            (new_sequence[0, idx_in_seq-1].item(), new_sequence[0, idx_in_seq].item()),
            starting_idx=idx_in_seq-1
        )
    if idx_in_seq < seqlen - 1: # add the bigram that starts here
        context_ngram.update(
            (new_sequence[0, idx_in_seq].item(), new_sequence[0, idx_in_seq+1].item()),
            starting_idx=idx_in_seq
        )
    return